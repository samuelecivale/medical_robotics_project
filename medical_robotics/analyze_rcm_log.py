...
--------------------------
Task 2 and Task 4 are NOT independent tests anymore.

Correct test workflow:
    1. Press 3 and wait until the insertion sequence reaches Done.
    2. Without resetting the scene, press 2 for the Target-RCM cone demo.
    3. Without resetting the scene, press 4 for the Entry-RCM tip-cone demo.
    4. Stop Play Mode.
    5. Run this script on the generated CSV.

The script separates the log into stages:
    T3_safe_insertion
    T2_target_rcm_cone_after_insertion
    T4_entry_rcm_tip_cone_after_insertion

If your controller writes an 'evaluation_stage' column, that column is used.
Otherwise, the script infers stages from the 'mode' column.
"""
...
COLUMN_ALIASES = {
    ...
    "mode": [
        "mode", "control_mode", "rcm_mode",
    ],
    "phase": [
        "phase", "insertion_phase",
    ],
    "evaluation_stage": [
...
SUCCESS_THRESHOLDS = {
    ...
    "max_needle_skull_violation_mm": 0.5,
...
def infer_stage(df: pd.DataFrame) -> pd.DataFrame:
    """
    Use explicit evaluation_stage if available.
    Otherwise infer stage from mode.

    Expected mode values:
        Double       -> T3_safe_insertion
        Target       -> T2_target_rcm_cone_after_insertion
        EntryTipCone -> T4_entry_rcm_tip_cone_after_insertion
    """
    out = df.copy()
...
def keep_after_insertion_done_for_post_tasks(df: pd.DataFrame) -> pd.DataFrame:
    """
    For T2/T4 interpretation, they should happen after T3 reaches Done.

    This function does NOT delete rows. It adds a boolean column:
        valid_post_insertion_context
    """
    out = df.copy()
...
def compute_metrics(df: pd.DataFrame) -> Dict[str, float | str | int | bool]:
    ...
    metrics["max_needle_skull_violation_mm"] = safe_max(df["needle_skull_violation_mm"])
    metrics["max_arm_skull_violation_mm"] = safe_max(df["arm_skull_violation_mm"])

    # -----------------------------------------------------------------
    # T3: main safe insertion sequence
    # -----------------------------------------------------------------
    ...
    if len(t3) > 0:
        ...
        # Fallback: if InsertToTarget was too short or not logged, use Done rows.
        insertion_eval = insert if len(insert) > 0 else done

        # Second fallback: use the last 20% of T3.
    ...
    # -----------------------------------------------------------------
    # T2: target-RCM cone demo, valid only after T3 Done
    # -----------------------------------------------------------------
    ...
    if len(t2) > 0:
        ...
        # In Target mode, target_error_mm is interpreted as target RCM / target constraint error.
    ...
    # -----------------------------------------------------------------
    # T4: entry-RCM tip-cone demo, valid only after T3 Done
    # -----------------------------------------------------------------
    ...
    if len(t4) > 0:
        ...
        # In EntryTipCone mode, entry_error_mm is the key RCM metric.
        ...
        # Target error is not expected to be zero because the tip moves around the target.
    ...
    # -----------------------------------------------------------------
    # Overall success flags
    # -----------------------------------------------------------------
    metrics["PASS_skull_safety"] = (
        metrics.get("max_arm_skull_violation_mm", math.inf) <= SUCCESS_THRESHOLDS["max_arm_skull_violation_mm"]
        and metrics.get("max_needle_skull_violation_mm", math.inf) <= SUCCESS_THRESHOLDS["max_needle_skull_violation_mm"]
    )
...
def plot_errors_over_time(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.plot(df["time"], df["needle_skull_violation_mm"], label="Needle skull violation")
    plt.plot(df["time"], df["arm_skull_violation_mm"], label="Arm skull violation")

    # Draw vertical lines when the stage changes.
    if "stage" in df.columns and len(df) > 1:
        ...
        for _, row in df.iterrows():
            ...
            if current != previous:
                ...
                plt.text(row["time"], plt.ylim()[1] * 0.95, current.replace("_", "\n"), rotation=90, va="top", fontsize=8)
                previous = current
...
def print_summary(metrics: Dict[str, float | str | int | bool]) -> None:
    ...
    print("\nInterpretation:")
    print("- T3 is the real surgical insertion task.")
    print("- T2 and T4 are post-insertion demonstrations and are valid only if executed after T3 reaches Done.")
    print("- Entry RCM error during ApproachEntry is not a failure: RCM becomes meaningful during insertion.")
    print("- Skull violation should stay close to zero for both needle and arm.")
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=None, help="Output directory for plots and summaries.")
    parser.add_argument("--warmup", type=float, default=0.0, help="Ignore the first N seconds of the log.")
    ...
    try:
        ...
        if args.warmup > 0:
            df = df[df["time"] >= args.warmup].copy()

        df = infer_stage(df)
        df = keep_after_insertion_done_for_post_tasks(df)
